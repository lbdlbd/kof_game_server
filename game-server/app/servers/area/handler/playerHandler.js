...
handler.enterScene = function(msg, session, next) {
  ...
  //create by client or other server(PVE)
  if(!!msg.teamId) {
    teamId = msg.teamId;
  }
  ...
  userDao.getPlayerAllInfo(playerId, function(err, player) {
    ...
    player.enterSceneTime = Date.now() / 1000;
		areaId = player.areaId;
		utils.myPrint("2 ~ GetPlayerAllInfo: player.instanceId = ", player.instanceId);
    utils.myPrint("2 ~ GetPlayerAllInfo: player.enterSceneTime = ", player.enterSceneTime);
    ...
    // temporary code
        ...
        subDungeonId: area.subDungeonId,
        damageCache: area.damageCache,
        attackCache: area.attackCache,
        map: {
          name : map.name,
          width: map.width,
          height: map.height,
...
handler.moveTo = function(msg, session, next) {
    ...
    var pos = {x : msg.x, y : msg.y};

    player.setPosition(msg.x,msg.y,msg.z);

    //Update the aoi module
    var watcher = {id : player.entityId, type : player.type};
    area.timer.updateObject(watcher, oldPos, pos);
    area.timer.updateWatcher(watcher, oldPos, pos, player.range, player.range);
    ...
    messageService.pushMessageByAOI(area, {
        ...
        speed:msg.speed,
        finish:msg.finish,
        faceLeft:msg.faceLeft
    }, pos, ignoreList);

    /*area.channel.pushMessage( 'onMoveTo',{
        entityId: player.getEntityId(),
        position: {x:msg.x, y:msg.y, z:msg.z},
        prediction:{px:msg.px, py:msg.py, pz:msg.pz},
        faceLeft:msg.faceLeft
    });*/
};
