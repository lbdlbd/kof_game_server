...
fightskillDao.getFightSkillsByPlayerId = function(playerId, cb) {
    pomelo.app.get('httpclient').query('/get_player_skill', {player_id: playerId}, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            for (var i = 0; i<res.length; i++){
                var result = res[i];
                console.log(result);
                var fightSkill = fightskill.create({id: result.id, playerId: result.player_id, skillId: 1, level: result.lv, type: 'attack'});
