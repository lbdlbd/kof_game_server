...
var toPlayer = function(data) {
    var kindId = (data.career == 1)?210:211;
    var player = {
        id : data.id,
        userId : data.accid,
        kindId : kindId,
...
/**
 * Get an user's all players by token
 * @param {Number} token User Token.
 * @param {function} cb Callback function.
 */
userDao.getPlayersByToken = function(token, cb){
    var query = {player_id: token.playerId, session: token.sessionId};
    pomelo.app.get('httpclient').query('/get_player_by_id', query, function (error, response, body) {
 ...
 * @param {function} cb Callback function.
 */
userDao.getPlayersByUid = function(userid, cb){
    pomelo.app.get('httpclient').query('/GetPlayersByUid', {userid: userid}, function (error, response, body) {
        if (error || response.statusCode != 200) {
            var err = error || new Error('statusCode: '+ response.statusCode);
            utils.invokeCallback(cb, err, null);
        } else {
...
userDao.getPlayerById = function(playerId, cb){
  pomelo.app.get('httpclient').query('/get_player_by_id', {player_id: playerId}, function (error, response, body) {
    if (error || response.statusCode != 200) {
      var err = error || new Error('statusCode: '+ response.statusCode);
      utils.invokeCallback(cb, err, null);
    } else {
      ...
      if (!res || res.length <= 0) {
        ...
        var player = toPlayer(res[0]);
        utils.invokeCallback(cb,null, new Player(player));
      }
    }
  });
};
...
userDao.getUserById = function (userid, cb){
    pomelo.app.get('httpclient').query('/GetUserById', {userid: userid}, function (error, response, body) {
        if (error || response.statusCode != 200) {
            var err = error || new Error('statusCode: '+ response.statusCode);
            utils.invokeCallback(cb, err, null);
        } else {
