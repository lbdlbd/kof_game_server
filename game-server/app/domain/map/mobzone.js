...
/**
 * The generate mob function, will generate mob, update aoi and push the message to all interest clients
 */
...
var genPoint = function(map, originX, originY, count) {
  ...
  var disx = Math.floor(Math.random() * 5) + 1;
  var disy = Math.floor(Math.random() * 0) + 1;
	var x, y;
	if(Math.random() > 0.5) {
		x = originX - disx;
	} else {
