...
var Instance = function(opts){
  this.instanceId = opts.instanceId;  //add
  this.subDungeonId = 0;  //add
  this.damageCache = {};  //add
  this.attackCache = {};  //add

  logger.debug('instanceId  %j %s', this.instanceId, this.dungeonProgress);
  this.areaId = opts.id;
  this.type = opts.type;
  this.map = opts.map;
...
Instance.prototype.addDamage = function(playerId,damage) {
  var length = damage.length;
  for(var i = 0; i < length; ++i) {
    var id = damage[i].id;
    var value = damage[i].value;

    if(!this.damageCache[id]) {
      this.damageCache[id] = 0;
    }
    this.damageCache[id] += value;

    if(!this.attackCache[playerId]) {
      this.attackCache[playerId] = 0;
    }
    this.attackCache[playerId] += value;
  }
};
 */
 ...
 * @api public
 */
Instance.prototype.addDamage = function(attacker, defender, damage) {
    if(defender > 0) {
        if (!this.damageCache[defender]) {
            this.damageCache[defender] = 0;
        }
        this.damageCache[defender] += damage;
    }

    if(attacker > 0) {
        if (!this.attackCache[attacker]) {
            this.attackCache[attacker] = 0;
        }
        this.attackCache[attacker] += damage;
    }
};
...
Instance.prototype.getChannel = function() {
  if(!this.channel){
    var channelName = channelUtil.getAreaChannelName(this.instanceId || this.areaId);
    utils.myPrint('channelName = ', channelName);
    logger.debug('channelName = %s %j', channelName, this.instanceId);
...
Instance.prototype.addEntity = function(e) {
  ...
  if(e.type === EntityType.PLAYER) {
    ...
    this.aoi.addWatcher({id: e.entityId, type: e.type}, {x : e.x, y: e.y}, e.range);
      logger.error('addWatcher  %j', e.range);
...
Instance.prototype.getAllUids = function() {
    var uids = [];
    for (var id in this.players) {
        var player = this.entities[this.players[id]];
        uids.push({uid: player.userId, sid: player.serverId});
    }
    return uids;
};
 ...
 * @param {Object} e Entity to add to the area.
 */
Instance.prototype.notifyAddEntity = function(e) {
    var entities = {};
    entities[e.type] = [e];
    this.channel.pushMessage( 'onAddEntities',entities);
};
